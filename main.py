...
def read():
    ...
    sql=f"SELECT `item_id`, `name`, `size`, `price`, `quantity`, `category`, `date` FROM stocks ORDER BY `id` DESC"
    cursor.execute(sql)
...
def save():
    ...
    valid=True
    if not(itemId and itemId.strip()) or not(name and name.strip())or not(size and size.strip()) or not(price and price.strip()) or not(qnt and qnt.strip()) or not(cat and cat.strip()):
    ...
    try:
        ...
        if len(checkItemNo) > 0:
            ...
            cursor.connection.ping()
            sql=f"INSERT INTO stocks (`item_id`, `name`, `size`, `price`, `quantity`, `category`) VALUES ('{itemId}','{name}','{size}','{price}','{qnt}','{cat}')"
            cursor.execute(sql)
        conn.commit()
        conn.close()
...
def update():
    ...
    try:
        cursor.connection.ping()
        sql=f"UPDATE stocks SET `name` = '{name}',`size`='{size}', `price` = '{price}', `quantity` = '{qnt}', `category` = '{cat}' WHERE `item_id` = '{itemId}' "
        cursor.execute(sql)
        conn.commit()
        conn.close()
...
def select():
    try:
        ...
        size = str(my_tree.item(selectedItem)['values'][2])
...
def find():
    itemId = str(itemIdEntry.get())
    name = str(nameEntry.get())
    size = str(nameEntry.get())
    price = str(priceEntry.get())
    qnt = str(qntEntry.get())
    cat = str(categoryCombo.get())
    cursor.connection.ping()
    if(itemId and itemId.strip()):
        sql = f"SELECT `item_id`, `name`, `price`, `size`, `quantity`, `category`, `date` FROM stocks WHERE `item_id` LIKE '%{itemId}%' "
    elif(name and name.strip()):
        sql = f"SELECT `item_id`, `name`, `price`, `size`, `quantity`, `category`, `date` FROM stocks WHERE `name` LIKE '%{name}%' "
    elif(size and size.strip()):
        sql = f"SELECT `item_id`, `name`, `price`, `size`, `quantity`, `category`, `date` FROM stocks WHERE `name` LIKE '%{size}%' "
    elif(price and price.strip()):
        sql = f"SELECT `item_id`, `name`, `price`, `size`, `quantity`, `category`, `date` FROM stocks WHERE `price` LIKE '%{price}%' "
    elif(qnt and qnt.strip()):
        sql = f"SELECT `item_id`, `name`, `price`, `size`, `quantity`, `category`, `date` FROM stocks WHERE `quantity` LIKE '%{qnt}%' "
    elif(cat and cat.strip()):
        sql = f"SELECT `item_id`, `name`, `price`, `size`, `quantity`, `category`, `date` FROM stocks WHERE `category` LIKE '%{cat}%' "
    else:
...
def exportExcel():
    cursor.connection.ping()
    sql=f"SELECT `item_id`, `name`, `price`, `size`, `quantity`, `category`, `date` FROM stocks ORDER BY `id` DESC"
    cursor.execute(sql)
...
entriesFrame.grid(row=1,column=0,sticky="w",padx=[30,200],pady=[0,20],ipadx=[7])

itemIdLabel=Label(entriesFrame,text="ITEM ID",anchor="e",width=10)
nameLabel=Label(entriesFrame,text="BRAND NAME",anchor="e",width=12)
sizeLabel=Label(entriesFrame,text="SIZE",anchor="e",width=10)
...
sizeLabel.grid(row=2,column=0,padx=10)
priceLabel.grid(row=3,column=0,padx=10)
qntLabel.grid(row=4,column=0,padx=10)
categoryLabel.grid(row=5,column=0,padx=10)

categoryArray=['Gold','Enduro','T-Master','Ultra MF','Electron','Primera'  ]
categoryArray1=['Golds','Enduro','T-Master','Ultra MF','Electron','Primera'  ]
categoryArray2=['Golds','Enduro','T-Master','Ultra MF','Electron','Primera'  ]

itemIdEntry=Entry(entriesFrame,width=50,textvariable=placeholderArray[0])
nameEntry=ttk.Combobox(entriesFrame,width=47,textvariable=placeholderArray[1],values=categoryArray1)
sizeEntry=ttk.Combobox(entriesFrame,width=47,textvariable=placeholderArray[2],values=categoryArray2)
...
sizeEntry.grid(row=2,column=2,padx=5,pady=5)
...
my_tree['columns']=("Item Id","Name","Size","Price","Quantity","Category","Date")
my_tree.column("#0",width=0,stretch=NO)
my_tree.column("Item Id",anchor=W,width=70)
my_tree.column("Name",anchor=W,width=125)
my_tree.column("Size",anchor=W,width=135)
...
my_tree.heading("Quantity",text="In Stocks",anchor=W)
